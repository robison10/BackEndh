...
class Database {
  ...
  async connect() {
    // ✅ Configurações robustas para MongoDB Atlas
    const mongoOptions = {
      ...
      authSource: 'admin',              // Source de autenticação
      ssl: true,                        // SSL/TLS
      tls: true                         // TLS
    };
    ...
    Console.log("Database", `📊 Database: ${this.dbName}`);
    Console.log("Database", `⚙️ Opções: ${JSON.stringify(mongoOptions, null, 2)}`);
    
    try {
      Console.log("Database", "🔄 Criando MongoClient...");
      this.client = new MongoClient(this.mongoUri, mongoOptions);
      
      Console.log("Database", "🔄 Conectando ao servidor...");
      await this.client.connect();
      Console.log("Database", "✅ Conexão estabelecida!");
      
      // ✅ Testar conexão com ping
      Console.log("Database", "🔄 Enviando ping...");
      const pingResult = await this.client.db('admin').command({ ping: 1 });
      Console.log("Database", `🏓 Ping bem-sucedido: ${JSON.stringify(pingResult)}`);
      
      Console.log("Database", `🔄 Selecionando database '${this.dbName}'...`);
      this.db = this.client.db(this.dbName);
      Console.log("Database", `✅ Database '${this.dbName}' selecionado!`);
      
      // ✅ Configurar coleções
      Console.log("Database", "🔄 Configurando coleções...");
      ...
      Console.log("Database", "✅ Coleções configuradas!");
      
      await this.createIndexes();
      Console.log("Database", "✅ Índices criados!");
      
      await this.autoPopulateSharedData();
      Console.log("Database", "✅ Dados compartilhados populados!");
      
      Console.log("Database", `✅ Conectado ao banco '${this.dbName}' com sucesso!`);
      
    } catch (error) {
      Console.error("Database", `❌ Erro na conexão: ${error.message}`);
...
// ✅ CONEXÃO ROBUSTA COM RETRY E DNS OTIMIZADO
async function connectDatabaseWithRetry() {
  ...
  while (retryCount < maxRetries) {
    try {
      Console.log("Database", `🔄 Tentativa ${retryCount + 1}/${maxRetries} de conexão ao MongoDB...`);
      Console.log("Database", `📍 URI: ${process.env.mongoUri.replace(/:[^:@]*@/, ':****@')}`);
      Console.log("Database", `📊 Database: ${database.dbName}`);
      
      // Aguardar um pouco entre tentativas
      if (retryCount > 0) {
        const waitTime = Math.min(1000 * Math.pow(2, retryCount), 10000); // Backoff exponencial
        Console.log("Database", `⏳ Aguardando ${waitTime/1000}s antes da próxima tentativa...`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
      }
      
      Console.log("Database", "🔄 Iniciando conexão...");
      ...
      retryCount++;
      Console.error("Database", `❌ Tentativa ${retryCount} falhou`);
      Console.error("Database", `📝 Mensagem: ${err.message}`);
      Console.error("Database", `🔍 Código: ${err.code}`);
      Console.error("Database", `📍 Erro completo: ${JSON.stringify(err, null, 2)}`);
      
      if (err.message.includes('ECONNREFUSED') || err.message.includes('querySrv')) {
        Console.warn("Database", "🔧 Problema de conectividade detectado (DNS/Rede)");
        
        if (retryCount === 2) {
          Console.log("Database", "🔄 Tentando DNS alternativos (Cloudflare)...");
          const dns = require('dns');
          dns.setServers(['1.1.1.1', '1.0.0.1']);
        } else if (retryCount === 3) {
          Console.log("Database", "🔄 Tentando DNS alternativos (OpenDNS)...");
          const dns = require('dns');
      ...
      if (err.message.includes('authentication failed')) {
        Console.error("Database", "🔐 Erro de autenticação - Verificar credenciais no .env");
      }
      
      if (err.message.includes('ENOTFOUND')) {
        Console.error("Database", "🌐 Erro de DNS - Host não encontrado");
      }
      
      if (retryCount >= maxRetries) {
        Console.error("Database", "❌ Falha ao conectar ao MongoDB após todas as tentativas");
        Console.error("Database", "🔧 Soluções possíveis:");
        Console.error("Database", "   1. Verificar se o IP está liberado no MongoDB Atlas");
        Console.error("Database", "   2. Verificar conexão com internet");
        Console.error("Database", "   3. Verificar firewall/antivírus");
        Console.error("Database", "   4. Tentar usar VPN");
        Console.error("Database", "   5. Verificar string de conexão no .env");
        Console.error("Database", "   6. Verificar credenciais (usuário/senha)");
        throw err;
      }
    }
  }
...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    // ✅ NOVO: Gerar ID sequencial baseado no maior ID existente
    const lastUser = await database.collections.Users.findOne({}, { sort: { id: -1 } });
    const userId = (lastUser?.id || 0) + 1;
    
    const username = 'RankedPlayer-' + BackendUtils.GenCaracters(4).toUpperCase();
    
    const user = {
      id: userId,
      deviceId,
      stumbleId: userId.toString(), // ✅ NOVO: StumbleId agora é sequencial (1, 2, 3, ...)
      username,
      country: 'RO',
      region: 'EU',
      ...
      // ✅ SecretKey para Discord Linking
      secretKey: `USER${userId}KEY${Math.floor(Date.now() / 1000)}`.substring(0, 16).toUpperCase()
    };
...
static async updateUsername(req, res) {
  try {
    const { Username, deviceId, userId } = req.body;
    let { user } = req;
    
    // Se não temos user do middleware, tentar buscar por deviceId ou userId
    if (!user && (deviceId || userId)) {
      if (deviceId) {
        user = await UserModel.findByDeviceId(deviceId);
      } else if (userId) {
        user = await UserModel.findById(parseInt(userId));
      }
    ...
    if (!user) {
      res.status(400).json({ message: 'User identification required (deviceId, userId, or authentication)' });
      return;
    }
    
    // Validação do formato do username
    if (!/^[a-zA-Z0-9_\-\s]+$/.test(Username)) {
      res.status(422).json({ message: 'Username can only contain letters, numbers, spaces, underscores and hyphens' });
      return;
    }
    
    if (Username.length < 3 || Username.length > 9999) {
      res.status(422).json({ message: 'Username must be between 3 and 20 characters' });
      return;
    }
    ...
    console.log(`✅ Username updated: ${user.stumbleId} -> ${Username}`);
